...
const measureButton = document.getElementById("measure-height");
...
async function encode(url) {
  ...
  img.src = url;
  img.onload = function () {
    // Set container size dynamically based on the uploaded image
    imageContainer.style.width = `${img.width}px`;
    imageContainer.style.height = `${img.height}px`;

    // Also adjust the mask canvas size
...
function measureMaskHeight() {
  if (!imageEmbeddings) {
    alert("Upload an image and segment it first!");
    return;
  }
  ...
  for (let y = 0; y < maskCanvas.height; y++) {
    for (let x = 0; x < maskCanvas.width; x++) {
      const index = (y * maskCanvas.width + x) * 4; // RGBA index
  ...
  maskContext.moveTo(maskCanvas.width / 2, topPixel); // Center X
  maskContext.lineTo(maskCanvas.width / 2, bottomPixel);
  maskContext.stroke();

  // Draw text showing height
  maskContext.fillStyle = "red";
  maskContext.font = "18px Arial";
  maskContext.fillText(
    `${maskHeight} px`,
    maskCanvas.width / 2 + 10,
    (topPixel + bottomPixel) / 2
  );

  statusLabel.textContent = `Mask Height: ${maskHeight} px`;
}
